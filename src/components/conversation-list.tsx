...
type Item = {
  id: string;
  other: { id: string; username: string; display_name: string | null; avatar_url: string | null; verified: boolean; last_seen_at: string } | null;
  last: { content: string | null; message_type: string; created_at: string; sender_id: string } | null;
  unread: number;
          ...
          >
            <div className="flex gap-3">
              <div className="relative shrink-0">
                <Avatar name={o?.display_name ?? o?.username ?? "?"} url={o?.avatar_url} size={40} />
              ...
              <div className="min-w-0 flex-1">
                <div className="mb-0.5 flex items-center justify-between">
                  <div className="flex items-center gap-1.5 truncate">
                    <span className="truncate text-sm font-medium text-foreground">{o?.display_name ?? o?.username}</span>
                    {o?.verified && <VerifiedBadge size={12} />}
                  </div>
                  <span className="ml-2 shrink-0 text-[10px] text-muted-foreground">{formatRelative(c.last_message_at)}</span>
                </div>
                <div className="flex items-center justify-between gap-2">
                  <p className={cn("truncate text-xs", c.unread > 0 ? "text-foreground" : "text-muted-foreground")}>{preview}</p>
                  {c.unread > 0 && (
