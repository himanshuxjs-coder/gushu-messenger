...
import { resolveUsernameToEmail, checkUsernameAvailable, ensureDemoUsers } from "@/lib/auth.functions";
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <Field label="Identity">
        <Input value={id} onChange={(e) => setId(e.target.value)} placeholder="email or username" autoComplete="username" required />
      </Field>
      <Field label="Credential">
        <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" autoComplete="current-password" required minLength={6} />
      </Field>
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <Field label="Username">
        <Input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="lowercase, 3–20 chars" pattern="[a-z0-9_]{3,20}" required />
      </Field>
      <Field label="Email">
        <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" required />
      </Field>
      <Field label="Password">
        <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="at least 8 characters" autoComplete="new-password" required minLength={8} />
      </Field>
      <Field label="Confirm password">
        <Input type="password" value={confirm} onChange={(e) => setConfirm(e.target.value)} placeholder="repeat password" autoComplete="new-password" required minLength={8} />
      </Field>
      <Button type="submit" disabled={loading} className="w-full">
...
function DemoCard() {
  ...
  return (
    <div className="mt-5 rounded-2xl bg-card/60 p-4 ring-1 ring-border">
      <div className="mb-3 text-[10px] font-medium uppercase tracking-[0.2em] text-muted-foreground">Try the demo</div>
