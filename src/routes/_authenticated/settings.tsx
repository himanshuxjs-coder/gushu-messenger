...
function SettingsPage() {
  ...
  const [me, setMe] = useState<{ id: string; username: string; display_name: string | null; bio: string | null; avatar_url: string | null; verified: boolean; created_at: string } | null>(null);
  ...
  if (!me) {
    return <div className="grid min-h-screen place-items-center"><Loader2 className="size-4 animate-spin" /></div>;
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-2xl space-y-8 px-6 py-10">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Joined {new Date(me.created_at).toLocaleDateString()} · @{me.username}{me.verified && " · verified"}</p>
        </div>
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="flex flex-1 flex-col gap-2 sm:flex-row">
              <label className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-md bg-foreground px-3 py-2 text-xs font-medium text-background hover:bg-foreground/90">
                ...
                <input type="file" accept="image/png,image/jpeg,image/webp" hidden onChange={(e) => { const f = e.target.files?.[0]; if (f) uploadAvatar(f); }} />
              </label>
              {me.avatar_url && (
        ...
        <section className="space-y-4 rounded-2xl bg-card p-6 ring-1 ring-border">
          ...
          <Button onClick={save} disabled={busy}>{busy ? <Loader2 className="size-4 animate-spin" /> : "Save changes"}</Button>
        </section>
      </main>
    </div>
  );
